...
Code to plot Figure1a in our paper "Misinformation can prevent the suppression of epidemics".
Pre-print available at: https://doi.org/10.1101/2021.08.23.21262494

The code solves the system of ODEs Eqs. (3-10) for different values of the parameter rho and then plots
...
rho = np.array([0.1, 0.2, 0.5, 0.65, 0.71, 0.8]) # efficiency of the NPIs
rr = 1/7.5 # recovery rate

dens = 0.1 # density of distrusting individuals in the population

p1 = len(rho) # length of array rho
...
def solveq(rho, dens, ml):
   ...
   tm = 160 # maximum time for which we solve the equations
...
if __name__ == '__main__':
    ...
    t_span = np.linspace(0,160,10001)
    ...
    plt.plot(t_span,traj[0,:].transpose(), linewidth=5, color='red',ms=15, marker='o',markevery=600,label=r'{0:.2f}'.format(rho[0]))
    plt.plot(t_span,traj[1,:].transpose(), linewidth=5, color='darkorange',ms=15, marker='^', markevery=600,label=r'{0:.2f}'.format(rho[1]))
    plt.plot(t_span,traj[2,:].transpose(), linewidth=5, color='gold',ms=15, marker='v', markevery=600,label=r'{0:.2f}'.format(rho[2]))
    plt.plot(t_span,traj[3,:].transpose(), linewidth=5, color='royalblue',ms=15, marker='D', markevery=600,label=r'{0:.2f}'.format(rho[3]))
    plt.plot(t_span,traj[4,:].transpose(), linewidth=5, color='darkblue',ms=15, marker='p', markevery=600,label=r'{0:.2f}'.format(rho[4]))
    plt.plot(t_span,traj[5,:].transpose(), linewidth=5, color='purple', ms=12,marker='s', markevery=600,label=r'{0:.2f}'.format(rho[5]))
    plt.axis([0,155,1E-6,0.5])
    ...
    fig.savefig("Figure1a.pdf", format='pdf')