...
Code to generate data to produce Figure 3 in our paper "Misinformation can prevent the suppression of epidemics".
Pre-print available at: https://doi.org/10.1101/2021.08.23.21262494

The code solves the system of ODEs Eqs. (3-10) for different values of the parameters rho d for the plots of Figure 3 
in the manuscript. The ODEs are solved in parallel using the solve_ivp function with the RK45 method.

The code is set to reproduce Figure 3c of the manuscript. The other figures can be obtained by changing the parameter 
values at the beginning of the code according to Figure 3's caption.

The repository contains the data obtained using this code for all cases.
"""
...
be = 2/7.5 # infection rate in the absence of behavioural changes
...
space1 = np.linspace(0.001,.99,101)
...
if __name__ == '__main__':
    ...
    contours = plt.contour(dens, rho, tp, colors='black')
    plt.clabel(contours, inline=True, fontsize=15,fmt='%.0f')
    im = plt.imshow(tp, extent=[0, 1, 0, 1], origin='lower',
               cmap='gnuplot2_r', alpha=.6)
    plt.xlabel(r'$d$')
    plt.ylabel(r'$\rho$')
    ...
    ax.text(0.1, 0.65, 'I', fontsize=75, style='italic')
    ax.text(0.5, 0.42, 'II', fontsize=75, style='italic')
