...
Code to plot Figures 2a and 2c in our paper "Misinformation can prevent the suppression of epidemics".
Pre-print available at: https://doi.org/10.1101/2021.08.23.21262494

The code solves the system of ODEs Eqs. (3-10) for different values of the parameters alpha_T and rho
...
# we solve the ODEs for several values of rho in (0,1)
rho = space1 # efficiency of the NPIs

# density of distrusting individual in the population
dens = 0.3
...
def solveq(rho, a1, ml):
   ...
   # solves system of ODEs for the values of alpha_T, rho and ml specified
   solv = solve_ivp(rhs, [0, tm], s0, method='RK45', t_eval=t_span, args=(a1, rho, ml,))
...
def task(k):
    ...
    # defines the task for the value of rho and alpha_T specified
    rec = solveq(rho[a], a1[b], ml)
...
if __name__ == '__main__':
    ...
    # defines the list of tasks, we want to solve the ODE system for each value of rho (p1 of them) and of alpha_T (p2 of them)
    ...
    # saves the data
    np.save('./varying_rho_ip.npy', ip)
    np.save('./varying_rho_tp.npy', tp)
    np.save('./varying_rho_fs.npy', fs)
    ...
    plt.plot(rho,ip[:,0], linewidth=3, color='red',ms=15, marker='o',markevery=5,label=r'{0:.2f}'.format(a1[0]))
    plt.plot(rho,ip[:,1].transpose(), linewidth=3, color='darkorange',ms=15, marker='^', markevery=5,label=r'{0:.2f}'.format(a1[1]))
    plt.plot(rho,ip[:,2].transpose(), linewidth=3, color='gold',ms=15, marker='v', markevery=5,label=r'{0:.2f}'.format(a1[2]))
    plt.plot(rho,ip[:,3].transpose(), linewidth=3, color='royalblue',ms=15, marker='D', markevery=5,label=r'{0:.2f}'.format(a1[3]))
    plt.plot(rho,ip[:,4].transpose(), linewidth=3, color='darkblue',ms=15, marker='p', markevery=5,label=r'{0:.2f}'.format(a1[4]))
    plt.plot(rho,ip[:,5].transpose(), linewidth=3, color='purple', ms=12,marker='s', markevery=5,label=r'{0:.2f}'.format(a1[5]))
    ...
    plt.setp(ax.spines.values(), linewidth=2)
    plt.plot(rho,fs[:,0], linewidth=3, color='red',ms=15, marker='o',markevery=5,label=r'{0:.2f}'.format(a1[0]))
    plt.plot(rho,fs[:,1].transpose(), linewidth=3, color='darkorange',ms=15, marker='^', markevery=5,label=r'{0:.2f}'.format(a1[1]))
    plt.plot(rho,fs[:,2].transpose(), linewidth=3, color='gold',ms=15, marker='v', markevery=5,label=r'{0:.2f}'.format(a1[2]))
    plt.plot(rho,fs[:,3].transpose(), linewidth=3, color='royalblue',ms=15, marker='D', markevery=5,label=r'{0:.2f}'.format(a1[3]))
    plt.plot(rho,fs[:,4].transpose(), linewidth=3, color='darkblue',ms=15, marker='p', markevery=5,label=r'{0:.2f}'.format(a1[4]))
    plt.plot(rho,fs[:,5].transpose(), linewidth=3, color='purple', ms=12,marker='s', markevery=5,label=r'{0:.2f}'.format(a1[5]))
    ax.xaxis.set_tick_params(width=2)
    plt.xticks(rotation=20)
    ax.yaxis.set_tick_params(width=2)
    plt.xlabel(r'$\rho$')
    ...
    plt.savefig('Figure2c.pdf', format='pdf')
    plt.show()